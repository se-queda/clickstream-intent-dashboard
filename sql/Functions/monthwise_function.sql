DROP FUNCTION IF EXISTS clickstream.get_monthwise_revenue(TEXT[], TEXT[], BOOLEAN, INT[], INT[], INT[], INT[]);
...
RETURNS TABLE (month TEXT, conversion_rate NUMERIC) AS $$
BEGIN
    RETURN QUERY
    SELECT s.month::TEXT, ROUND(100.0 * sum(CASE WHEN s.revenue THEN 1 ELSE 0 END) / NULLIF(count(*), 0), 2)
