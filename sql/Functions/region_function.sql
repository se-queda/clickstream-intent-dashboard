...
RETURNS TABLE (name TEXT, conversions BIGINT, conversion_rate NUMERIC) AS $$
BEGIN
    RETURN QUERY
    SELECT
        r.name::TEXT,
    ...
    FROM clickstream.shopper_data s JOIN clickstream.dim_region r ON s.region = r.id
    ...
    GROUP BY r.name ORDER BY conversions DESC;
END;
$$ LANGUAGE plpgsql;