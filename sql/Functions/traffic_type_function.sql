...
BEGIN
    RETURN QUERY
    SELECT s.traffic_name, ROUND(100.0 * sum(CASE WHEN s.revenue THEN 1 ELSE 0 END) / NULLIF(count(s.traffictype), 0), 2)
    ...
    GROUP BY s.traffic_name ORDER BY conversion_rate DESC;
END;
$$ LANGUAGE plpgsql;