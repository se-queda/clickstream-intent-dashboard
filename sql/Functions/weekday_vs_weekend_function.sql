DROP FUNCTION IF EXISTS clickstream.get_weekday_vs_weekend(TEXT[], TEXT[], BOOLEAN, INT[], INT[], INT[], INT[], TEXT[]);
CREATE OR REPLACE FUNCTION clickstream.get_weekday_vs_weekend(
...
RETURNS TABLE (weekend BOOLEAN, conversion_rate NUMERIC) AS $$
BEGIN
    RETURN QUERY
    SELECT s.weekend, ROUND(100.0 * sum(CASE WHEN s.revenue THEN 1 ELSE 0 END) / NULLIF(count(*), 0), 2)
